...
> ⚠️ **重要声明**：
> - 本项目尚未在独立环境重新验证过，部署过程中可能会遇到各种问题，不保证一次性配置就能跑通。请尽量使用Clade Code等Coding Agent进行辅助部署。项目是纯Vibe Coding的结果，作者一行代码也没看过，暂时没有计划长期维护开源版本，所以PR 不会被处理，仅供参考。

本项目是一套基于AI的信息聚合与自动化推送系统，主要功能包括公众号聚合、播客更新监控，社区热点追踪以及投资数据分析。
![](images/all.png)
...
⚠️ **强烈建议**：使用 Claude Code 等 AI 工具读取 `CLAUDE.md`、`agents/` 和 `DESENSITIZATION_GUIDE.md` 来辅助部署。由于项目是纯vibe codingg的结果，进行信息脱敏之后没有进行额外测试，AI 工具能帮助你理解配置方法和常见问题解决方案。
...
1. **不要提 PR**：本项目是纯Vibe Coding的结果，作者一行代码也没看过，作者暂时没有计划长期维护开源版本，所以PR 不会被处理，仅供参考。
